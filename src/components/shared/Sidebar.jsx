  ...
  return (
    <>
      ...
      <div className={`fixed inset-0 flex z-40 md:hidden ${open ? 'block' : 'hidden'}`}>
        {/* Overlay */}
        <div 
          className={`fixed inset-0 bg-gray-600 bg-opacity-75 transition-opacity ${open ? 'opacity-100' : 'opacity-0'}`}
          onClick={() => setOpen(false)}
        ></div>
        
        {/* Sidebar */}
        <div className={`relative flex-1 flex flex-col max-w-xs w-full bg-white dark:bg-secondary transform transition ease-in-out duration-300 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
